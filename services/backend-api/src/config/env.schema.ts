...
console.log('🔍 DEBUG: process.env keys after dotenv:', Object.keys(process.env).filter(key => key.includes('DATABASE') || key.includes('JWT') || key.includes('COOKIE') || key.includes('REDIS') || key.includes('STRIPE')));
