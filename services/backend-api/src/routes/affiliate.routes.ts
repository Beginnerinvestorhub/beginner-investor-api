...
import { requireAffiliateAccess, trackAffiliate } from '../middleware/affiliate.middleware';
...
router.get('/dashboard', requireAuth, requireAffiliateAccess, async (req, res) => {
  try {
    // @ts-ignore - user is attached to request by auth middleware
    const userId = req.user.id;
...
router.post('/generate-link', requireAuth, requireAffiliateAccess, async (req, res) => {
  try {
    const { campaign } = req.body;
    // @ts-ignore - user is attached to request by auth middleware
    const userId = req.user.id;
    
    const link = await affiliateService.generateAffiliateLink(userId, campaign);
...
router.get('/referrals', requireAuth, requireAffiliateAccess, async (req, res) => {
  try {
    // @ts-ignore - user is attached to request by auth middleware
    const userId = req.user.id;
