...
export function createRateLimitMiddleware(options: RateLimitOptions) {
  return async (req: Request, res: Response, next: NextFunction) => {
    try {
      const result = await getRateLimiter().check(req, options);
 ...
 */
export const marketDataRateLimit = createRateLimitMiddleware(
 ...
 */
export const aiEngineRateLimit = createRateLimitMiddleware(
  RATE_LIMITS.AI_ENGINE,
);
