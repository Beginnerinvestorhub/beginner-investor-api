...
describe("ProgressService", () => {
  ...
  beforeEach(() => {
    ...
    mockPrismaGamificationResponses.userProgress.upsert.mockResolvedValue(
      mockProgress,
    );
    mockPrismaGamificationResponses.userProgress.findUnique.mockResolvedValue(
      mockProgress,
    );
  });
  ...
  describe("addExperience", () => {
    it("should add experience to a user's progress", async () => {
      ...
      mockPrismaGamificationResponses.userProgress.upsert.mockResolvedValueOnce(
        updatedProgress,
      );
      ...
      expect(
        mockPrismaGamificationResponses.userProgress.upsert,
    ...
    it("should level up when enough experience is gained", async () => {
      ...
      mockPrismaGamificationResponses.userProgress.findUnique.mockResolvedValueOnce(
        existingProgress,
      );
      mockPrismaGamificationResponses.userProgress.upsert.mockResolvedValueOnce(
        updatedProgress,
      ...
      expect(result).toEqual({
        ...updatedProgress,
        xpToNextLevel:
          calculateXPForLevel(updatedProgress.level) -
    ...
    it("should get the progress for a user", async () => {
      ...
      expect(mockRedisService.set).toHaveBeenCalledWith(
        ...
        JSON.stringify({
          ...mockProgress,
          xpToNextLevel:
            calculateXPForLevel(mockProgress.level) - mockProgress.experience,
        }),
        300, // 5 minutes TTL
      );
