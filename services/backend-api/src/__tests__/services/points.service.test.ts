...
describe("PointsService", () => {
  ...
  beforeEach(() => {
    ...
    mockPrismaGamificationResponses.pointTransaction.create.mockResolvedValue(
      mockTransaction,
    );
    mockPrismaGamificationResponses.pointTransaction.findMany.mockResolvedValue(
      [mockTransaction],
    );
  ...
  describe("awardPoints", () => {
    it("should award points to a user", async () => {
      ...
      expect(
        mockPrismaGamificationResponses.pointTransaction.create,
  ...
  describe("getUserPoints", () => {
    it("should get total points for a user", async () => {
      ...
      expect(
        mockPrismaGamificationResponses.pointTransaction.aggregate,
      ).toHaveBeenCalledWith({
  ...
  describe("getUserTransactions", () => {
    it("should get paginated transactions for a user", async () => {
      ...
      const result = await pointsService.getUserTransactions(mockUser.id, {
        page,
        limit,
      });

      expect(
        mockPrismaGamificationResponses.pointTransaction.findMany,
      ).toHaveBeenCalledWith({
        where: { userId: mockUser.id },
  ...
  describe("getLeaderboard", () => {
    ...
    it("should return cached leaderboard if available", async () => {
      ...
      mockRedisService.get.mockResolvedValueOnce(
        JSON.stringify(cachedLeaderboard),
      );
