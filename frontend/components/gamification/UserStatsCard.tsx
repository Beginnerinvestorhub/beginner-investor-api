...
export default function UserStatsCard({ 
  ...
  const currentLevelPoints = level > 1 ? 
    [0, 100, 250, 450, 700, 1000, 1350, 1750, 2200, 2700, 3250][level - 1] || 0 : 0;
  const nextLevelPoints = [100, 250, 450, 700, 1000, 1350, 1750, 2200, 2700, 3250, 4000][level - 1] || 4000;
      ...
      <div className={`bg-white rounded-lg shadow-md p-4 ${className}`}>
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center space-x-3">
            ...
            <div>
              <div className="font-semibold text-gray-900">{totalPoints} pts</div>
    ...
    <div className={`bg-white rounded-xl shadow-lg p-6 ${className}`}>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
        ...
        <div className="text-center p-3 bg-purple-50 rounded-lg">
          <div className="text-2xl font-bold text-purple-600">{streaks.loginStreak}</div>
          <div className="text-sm text-purple-500">Login Streak</div>
        </div>
        
        <div className="text-center p-3 bg-yellow-50 rounded-lg">
          <div className="text-2xl font-bold text-yellow-600">{stats.toolsUsed.length}</div>
      ...
      <div className="mt-6 pt-6 border-t border-gray-200">
        <div className="grid grid-cols-2 gap-4 text-sm text-gray-600">
          <div>
            <span className="font-medium">Risk Assessments:</span> {stats.assessmentsCompleted}
          </div>
          <div>
            <span className="font-medium">Portfolios Created:</span> {stats.portfoliosCreated}
          </div>
          <div>
            <span className="font-medium">Learning Streak:</span> {streaks.learningStreak} days
          </div>
          <div>
            <span className="font-medium">Education Modules:</span> {stats.educationModulesCompleted}
          </div>
        </div>
      </div>
    </div>
